...
const toSongDetail = function (id) {
    console.log("FindSongOne  id   ,", id)
    router.push({
        path: '/ConSongDetail',
        query: {
            id: id,
    ...
    <div class="container">
        ...
        <div class="imgSlide">
            <van-swipe :loop="false" :width="100" :show-indicators="false">
                <van-swipe-item v-for="(item) in arr" :key="item.id" @click.stop="toSongDetail(item.id)">
                    <div class="imgCon">
                        <div class="itemImg">
                        ...
                        <div>
                            <span style="font-size: .22rem; line-height: 1;">x{{ item.name }}</span>
                        </div>
                    </div>
